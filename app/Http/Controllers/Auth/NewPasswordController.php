...
class NewPasswordController extends Controller
{
    ...
    public function processUpdate(Request $request)
    {
        ...
        $request->validate([
            'email' => 'required|email',
            'password' => 'required|confirmed',
            'token' => 'required', // Đảm bảo rằng trường token được gửi qua request
        ]);
        ...
        try {
            ...
            if (
                ...
                Carbon::parse($passwordResetToken->created_at)->lte(Carbon::now()->subMinutes(30))
            ) {
                return redirect('/')->with('mess','Your link has expired');
            }

            // Tìm và cập nhật mật khẩu người dùng
            $account = Accounts::where('email', $request->email)->firstOrFail();
